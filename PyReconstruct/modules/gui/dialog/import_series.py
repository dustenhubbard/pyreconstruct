...
class ImportAlignmentsDialog(QDialog):
    ...
    The same alignments are also reachable through ImportSeriesDialog's
    Alignments tab, which is where they used to live exclusively. That dialog
    is the whole-series merge: eight tabs, most of them about traces. A user
    who only wants a colleague's alignment had to find it in there, so
    Alignments > Import alignments offers this one-tab version too. Both run
    the same MultiImportAs widget and therefore the same overwrite prompt.
    """
...
def collidingImportNames(entries, existing) -> list:
    ...
    Kept as a plain function (no Qt) because it is the whole decision behind
    the overwrite prompt: warn only about names that really are already taken,
    and name them. Duplicates are collapsed so a two-row import onto one
    existing name reads as one collision.

        Params:
            entries (list): (source name, target name) pairs
            existing (iterable): the names already present in the series
        Returns:
        ...
        """Create the multi line edit widget.
            ...
            Params:
                ...
                allow_overwrite (bool): True if a name that already exists may
                    ...
                    name outright, which is the historical behavior.
                default_to_source (bool): True if each row's target name
                    should prefill from its source name
        """
        super().__init__(parent)
