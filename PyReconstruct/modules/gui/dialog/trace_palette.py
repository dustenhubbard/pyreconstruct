...
def parseTags(text : str):
    ...
    The cell is edited in place, so a partial delete routinely leaves a stray
    separator behind: taking "spine" out of "axon, spine" leaves "axon, ", and
    an untagged trace starts out as "". Splitting on ", " alone turned both of
    those into an empty tag, because "".split(", ") is [""], and that empty tag
    was then written to the palette and offered in the tag filters. Split on the
    comma itself, strip each piece, and drop what is left empty, so deleting
    part of the text does what it looks like it does.
