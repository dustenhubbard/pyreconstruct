...
class ColorButton(QPushButton):
    ...
    def selectColor(self):
        ...
        you click a color and the thing you are coloring changes immediately,
        ...
        invalid ``QColor`` and the color that was picked is dropped on the
        ...
        color panel on the machine, so the confirming click is the one step
        ...
        button is inside its own window, so "picked a color and left" is not a
        gesture it offers. It is also what Windows and Linux users already get,
        which is why this is the behavior worth converging on rather than a
        macOS special case.

        The color is still only committed on an accepted dialog: dismissing it
        ...
        routes its initial color into the platform helper; turning
        ...
        trace's color is simply gone. A user who opened the picker on a green
        trace, saw the color was fine and pressed OK got a white trace and a
        blank swatch -- the reported bug again, by another route, and worse,
        because this one *writes* the wrong color rather than dropping the
        ...
        the same invalid color, so "cancelled on purpose", "picked a color the
        dismissal discarded", and "OK misbehaved" were indistinguishable. A
        dismissal's line records the color the picker was showing at that
        moment, which is the datum that separates them:

        - ``dismissed without OK ... showing rgb(0,255,0)``: the picked color
        ...
        - ``accepted but the color is invalid``: the platform dialog broke its
    ...
    def _logPickerOutcome(confirmed, color, showing, explicit_cancel, widget_dialog):
        ...
        if confirmed and color.isValid():
            log_note(
                "Color picker: accepted, applying "
                f"rgb({color.red()},{color.green()},{color.blue()})"
            )
        elif confirmed:
            # Accepted dialogs return their current color, which QColor can
            # always represent -- so this line means the color dialog broke
            # its own contract, and a report containing it should go to Qt.
            log_note(
                "Color picker: accepted but the color is invalid -- "
                f"platform dialog fault, nothing applied (was showing {shown})"
            )
        elif explicit_cancel:
            log_note(
                f"Color picker: Cancel pressed, nothing applied (was showing {shown})"
            )
        elif widget_dialog:
            log_note(
                "Color picker: dismissed without OK or Cancel (title-bar "
                f"close or Esc) while showing {shown} -- that color was NOT "
                "applied; only OK applies a color"
            )
        else:
            ...
            log_note(
                f"Color picker: dismissed (no Cancel button) while showing {shown} "
                "-- that color was NOT applied; only OK applies a color"
            )
