...
- **Changed:** Menu items in File and Series now say what they act on. "Reload" is now "Restart PyReconstruct" — it does restart the whole program, reloading it and reopening your series, which the old name did not tell you. "Open" is "Open series...", "Open recent" is "Open recent series", "Close" is "Close series" (it returns you to the welcome series rather than quitting), and "New" is "New series". In the Series menu, "Update curation from history" is now "Restore object curation status from log": it reads your series log and puts back curation statuses that the series itself has lost, without ever clearing a status or undoing an object you have already marked curated. Nothing moved, nothing was removed and no keyboard shortcut changed.
...
- **Changed: Right-click menus put what you actually do at the top.** In the field, Edit, Merge, Merge attributes only and Hide are now one click instead of one level down under "Trace"; on an object, Comment, Duplicate object, Hide, Add to 3D scene and Groups are all one click; and every list (objects, traces, z-traces, sections, flags) opens with a real action instead of "Invert selection", which moved down beside "Copy values". Nothing was removed and no keyboard shortcut changed — things only moved closer to hand.
