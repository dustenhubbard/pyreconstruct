...
# standalone (it installs from a configurable pip source, defaulting to the fork).
...
# The fork — never upstream SynapseWeb.
DEFAULT_SOURCE="git+https://github.com/dustenhubbard/PyReconstruct.git"
...
if [ "$MODE" = "uninstall" ]; then
  ...
  note "  curl -fsSL https://raw.githubusercontent.com/dustenhubbard/PyReconstruct/main/packaging/linux/uninstall.sh | bash"
...
install_icon() {
  local src
  src=$("$VENV/bin/python" -P -c 'import PyReconstruct, os; print(os.path.join(os.path.dirname(PyReconstruct.__file__), "assets", "img", "PyReconstruct.png"))' 2>/dev/null) || src=""
  if [ -n "$src" ] && [ -f "$src" ]; then
