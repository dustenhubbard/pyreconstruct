...
Undoes the last action you made on the trace field. Actions made through the object, trace, ztrace, flag, and section lists can be undone as well. If the action changed more than one section, PyReconstruct asks whether to undo it on all affected sections or only the current one.
...
Allows the user to pick a new shape for all traces associated with the selected objects.

Like Edit Radius, this should **only** be done with objects that are known to be stamps.
