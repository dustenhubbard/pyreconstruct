"""Regression test for upstream issue #99.
...
set names the merge's contours -- and not ``<obj>_split``, which did not exist
...
a real ``Section``/``Series`` from the checked-in fixture, and a real
...
# Section 48 of the fixture series carries four distinct objects, each with one
# trace: the focused object, the victim, and two bystanders that must come
# through untouched.
SNUM = 48
...
def field(real_series):
    """A stub field over section 48 of the fixture series, focused on FOCUS_OBJ,
    with one edit already recorded so that undo takes its multi-state path."""
    series = real_series
...
def test_bystanders_untouched_by_split_and_undo(field):
    ...
    assert "d03sp13" not in before or after["d03sp13"] == before["d03sp13"]
