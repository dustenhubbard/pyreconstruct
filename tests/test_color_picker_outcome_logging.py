"""How the color picker was closed must be readable from the log afterwards.
...
``QColorDialog`` as the same invalid color, and ``ColorButton.selectColor``
then (correctly, for Cancel) does nothing, silently. "The user cancelled",
"the user picked a color and the dismissal discarded it", and "OK itself
...
was closed, and a dismissal's line records the color the picker was showing
...
rgb(0,255,0)`` (the color reached the dialog and the dismissal dropped it),
or ``accepted, applying rgb(0,255,0)`` (the picker did its job and the fault
is in the swatch), or an ``accepted but the color is invalid`` line (the
...
platform this suite runs on has no native color dialog, so it builds the Qt
widget dialog through the identical no-platform-helper branch Windows takes
(Qt 6.5.2 ships no native color dialog on Windows; the ``ChooseColorW``
...
class DrivenColorDialog(QColorDialog):
    ...
    ``selectedColor()``'s set-on-accept/invalid-on-reject behavior -- is Qt's
...
def _picker_lines(capsys):
    ...
    return [line for line in err.splitlines() if "Color picker:" in line]


# --- one line per interaction, naming how the dialog was closed ------------


def test_ok_logs_the_applied_color(pick_with, parent, capsys):
    pick_with(press_ok)
    ...
    # and the behavior the line reports actually happened
...
def test_dismissal_records_the_color_it_discarded(pick_with, parent, capsys):
    ...
    assert "rgb(0,255,0)" in lines[0], (
        "the dismissal line must record the color the picker was showing, "
        "or it cannot distinguish 'discarded a picked color' from 'closed "
        "an untouched picker'"
    )
...
# --- the logging is an observer: behavior itself is unchanged -------------
...
def test_cancel_still_leaves_the_color_alone(pick_with, parent):
    pick_with(press_cancel)
    button = ColorButton(RED, parent)
...
def test_picker_still_opens_on_the_buttons_color(pick_with, parent):
