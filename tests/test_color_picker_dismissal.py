"""``ColorButton`` must own the color picker it opens.
...
a color, close the panel". Driven exactly that way through the real widget --
...
and nothing about green was special -- the color simply never entered the
application, for any color, on any dismissal that was not that one button.
...
dialog turned off, parented to the button. Plus the two end-to-end behaviors
that property exists to protect -- an accepted color reaches both the stored
attribute and the painted swatch, and a dismissed one changes neither.

The offscreen platform used by the suite has no native color dialog, so a test
...
"the picker must open on the color it is editing" are the answer to it.
Switching off the native dialog is only half the job: the Qt dialog has to be
seeded with the color the swatch already holds, and on macOS a dialog
...
original, which at least left the stored color alone. Because offscreen has no
...
class RecordingColorDialog(QColorDialog):
    ...
    ``setOption`` reproduces one cocoa behavior the suite's platform cannot
    ...
    def setOption(self, option, on=True):
        ...
        allowed routes its initial color into the platform helper. Turning
        ...
        offscreen platform the suite runs on has **no** native color dialog, so
        there the flip is a no-op and every constructor ordering preserves the
        color. Measured, same script, ``QT_QPA_PLATFORM=offscreen``: all four
    ...
    def exec(self):
        ...
        # which is the gesture that loses the color when the seeding is wrong.
...
def _painted_channel(button):
    """Which channel dominates the color actually painted on the swatch.

    Not an exact pixel match, deliberately. The swatch is a ``QPushButton``
    whose color is a ``background-color`` stylesheet, and how much of the
    button's own painting survives on top of it is a style decision: under the
    macOS style the centre pixel is the requested color exactly, while under
    ...
    the part that is about the color rather than about the style, and it is
...
def test_picker_is_a_dialog_this_code_owns(picker, parent):
    ...
    assert opened["options"] & QColorDialog.ColorDialogOption.DontUseNativeDialog, (
        "the picker may use the platform's color panel, whose close button "
        "discards the color the user picked"
    )
...
# --- the picker must open on the color it is editing ----------------------
#
# Switching the picker off the platform's panel is only half the job: the Qt
# dialog has to be *seeded* with the color the swatch already holds, and the
...
# it, see the color is fine, press OK -- silently repaints the trace white.
# From the user's chair that is the reported bug again: the swatch went blank.
# Worse, in fact, than the original, which at least left the stored color
# alone; this writes a wrong one.


def test_picker_opens_on_the_buttons_current_color(picker, parent):
    """Not on white: the color the user is about to edit."""
    button = ColorButton(RED, parent)

    button.selectColor()

    assert picker.execs[0]["currentColor"] == RED, (
        "the picker opened on the wrong color, so the trace's own color is "
        "not what the user is offered -- set DontUseNativeDialog before "
        "seeding the color, not after"
    )


def test_ok_without_changing_anything_leaves_the_color_alone(picker, parent):
    """The whole point: OK on an untouched picker is not a color change."""
    ...
    assert button.getColor() == RED, (
        "pressing OK without touching anything rewrote the trace's color"
    )
...
def test_accepted_color_is_stored_and_painted(picker, parent):
    button = ColorButton(None, parent)
...
def test_accepted_color_replaces_an_existing_one(picker, parent):
    button = ColorButton(RED, parent)
...
def test_dismissed_picker_leaves_an_existing_color_alone(picker, parent):
    picker.result_code = QDialog.DialogCode.Rejected
    button = ColorButton(RED, parent)
