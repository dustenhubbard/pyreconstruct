...
``Series > Import series data > From another series``, the eight-tab whole-series
merge dialog, while ``Alignments > Import alignments`` offered ".txt" and SWiFT
only. The three sources now sit together, behind
``MainWindow.importAlignmentsFromSeries``.

Three things get guards, because each one is a claim the diff makes:
...
2. The handler runs the import it advertises: it opens the chosen series,
   honours a cancelled calibration check and a cancelled dialog, hands
...
the handler tests call the unbound method against a stub carrying just the
surface it touches, the same idiom as
``tests/test_copy_to_sections_review.py``.
...
def test_overwrite_is_rejected_outright_when_not_allowed(qapp, monkeypatch):
    """The historical behavior, unchanged for palettes and b/c profiles."""
